...
import com.example.eventelevate.Adapter.FirebaseListAdapter;
...
import com.example.eventelevate.Model.MessageEntry;
import com.example.eventelevate.Model.MessageModel;
import com.example.eventelevate.R;
import com.example.eventelevate.Service.RetrofitClient;
import com.example.eventelevate.databinding.ActivityFirebaseBinding;
...
public class FirebaseActivity extends AppCompatActivity {
    ...
    private void getDataFromIntent() {
        ...
        if (intent != null) {
            ...
            clientId = Integer.parseInt(intent.getStringExtra("clientId"));
            ...
            public void onResponse(Call<LoginModel> call, Response<LoginModel> response) {
                if (response.body().getStatusCode() == 200) {
                    if (response.body().getMessage().trim().equals("User details retrieved successfully")) {
                        binding.otherUserNameText.setText(response.body().getUser().getFirstName() + " " + response.body().getUser().getLastName());
                        Glide.with(FirebaseActivity.this).load(response.body().getUser().getPhoto()).into(binding.userProfileImage);
                    }
